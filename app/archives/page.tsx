...
import { Filter, ArrowUpDown, Archive, ArrowLeft } from 'lucide-react';
...
export default function Archives() {
    ...
    const fetchItems = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/${tab}?archived=true`, {
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-slate-700">
                                {loading ? (
                                    ...
                                    <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500">No archived items found</td></tr>
                                ) : (
                                    filteredItems.map(item => (
                                        <tr
                                            key={item.id}
                                            onClick={() => router.push(`/item/${item.id}?type=${tab === 'bugs' ? 'bug' : 'feature'}`)}
                                            className="hover:bg-slate-700/50 cursor-pointer transition-colors opacity-75"
                                        >
                                            <td className="px-6 py-4"><StatusBadge severity={tab === 'bugs' ? item.severity : item.priority} muted={true} /></td>
                                            <td className="px-6 py-4 font-medium text-slate-400">{item.title}</td>
